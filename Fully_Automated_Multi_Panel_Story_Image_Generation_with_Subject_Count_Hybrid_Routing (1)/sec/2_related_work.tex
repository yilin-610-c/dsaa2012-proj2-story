...
IP-Adapter~\cite{ye2023ip-adapter} injects reference-image features into diffusion cross-attention, enabling lightweight identity control without full model fine-tuning.
...
StoryDiffusion~\cite{zhou2024storydiffusion} proposes consistent self-attention to share identity information across a batch of related prompts, improving multi-panel coherence.
...
PixArt-$\alpha$~\cite{chen2023pixartalpha} extends DiT with T5 text encoding and cross-attention for text-to-image synthesis, with a mature DreamBooth LoRA ecosystem~\cite{ruiz2022dreambooth}.
...
CLIP~\cite{radford2021learning} provides joint image--text embeddings; we use a ViT-B/32 variant to score candidate panels against generation, action, and previous-panel consistency prompts when multiple candidates are generated per scene.
