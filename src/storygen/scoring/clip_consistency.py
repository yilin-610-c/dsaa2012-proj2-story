...
class CLIPConsistencyScorer(BaseScorer):
    ...
    def score_candidate(
        ...
        *,
        ...
        action_critical = bool(candidate.metadata.get("action_critical", False))
        ...
        active_route_aware_config = self._route_aware_config(
            route_change_level,
            action_critical=action_critical,
        )
        if active_route_aware_config is not None and previous_image_path is not None:
            effective_weights["previous_image_consistency"] = active_route_aware_config["consistency_weight"]
    ...
    def _route_aware_config(self, route_change_level: Any, *, action_critical: bool) -> dict[str, Any] | None:
        ...
        action_critical_weights = route_aware.get("consistency_weight_by_change_level_when_action_critical", {})
        ...
        base_consistency_weight = float(
            consistency_weights.get(route_change_level, self.scoring_config.get("consistency_weight", 0.35))
        )
        action_critical_override_applied = False
        effective_consistency_weight = base_consistency_weight
        if action_critical and route_change_level in action_critical_weights:
            effective_consistency_weight = float(action_critical_weights[route_change_level])
            action_critical_override_applied = True
        return {
            "enabled": True,
            "route_change_level": route_change_level,
            "action_critical": action_critical,
            ...
            "action_critical_override_applied": action_critical_override_applied,
