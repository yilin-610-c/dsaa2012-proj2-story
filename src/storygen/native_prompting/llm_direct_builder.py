...
class LLMDirectPromptBuilder:
    ...
    def _build_messages(self, story: Story) -> list[dict[str, str]]:
        ...
        user_prompt = (
            ...
            "- action_prompt is required and must be a short visible-action phrase. For action-critical scenes it must distinguish success from near-miss candidates. For non-action-critical scenes, keep it clear but do not over-expand it.\n"
            "- scoring_prompt is required and must be short but concrete; include the key action and important visible evidence, not only the subject and nearby setting. For non-action-critical scenes, keep it concise and do not over-expand it.\n"
            "Rules:\n"
